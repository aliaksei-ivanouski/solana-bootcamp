...
export type DecrementInstruction<
  ...
  TAccountBootcampdapp extends string | AccountMeta<string> = string,
  TRemainingAccounts extends readonly AccountMeta<string>[] = [],
> = Instruction<TProgram> &
  InstructionWithData<ReadonlyUint8Array> &
  InstructionWithAccounts<
    [
      TAccountBootcampdapp extends string
        ? WritableAccount<TAccountBootcampdapp>
        : TAccountBootcampdapp,
      ...TRemainingAccounts,
    ]
  >;
...
export type DecrementInput<TAccountBootcampdapp extends string = string> = {
  bootcampdapp: Address<TAccountBootcampdapp>;
};
...
>(
  input: DecrementInput<TAccountBootcampdapp>,
  config?: { programAddress?: TProgramAddress }
): DecrementInstruction<TProgramAddress, TAccountBootcampdapp> {
  // Program address.
  const programAddress = config?.programAddress ?? BOOTCAMPDAPP_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    bootcampdapp: { value: input.bootcampdapp ?? null, isWritable: true },
  };
  ...
  const instruction = {
    accounts: [getAccountMeta(accounts.bootcampdapp)],
    programAddress,
    data: getDecrementInstructionDataEncoder().encode({}),
  } as DecrementInstruction<TProgramAddress, TAccountBootcampdapp>;
